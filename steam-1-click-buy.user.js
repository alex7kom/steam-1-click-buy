...
/* global jQuery:false, FinalizeTransaction:false, ShowAlertDialog:false */

(function () {

  function main () {
    var debug = Boolean(localStorage.getItem('1click_debug'));
    ...
    var async = {
      series: function (array, callback) {
        this.eachSeries(array, function (item, index, next) {
          item(next);
        }, callback);
      },
      eachSeries: function (array, iterator, callback) {
        ...
        (function next (error) {
          ...
          setTimeout(function () {
            iterator(array[index], index, next);
          }, 0);
        })();
      }
    };
    ...
    function emptyCart (itemIds, callback) {
      async.eachSeries(itemIds, function (itemId, index, cb) {
    ...
    function validateCart (callback) {
      ...
      async.eachSeries(cartRows, function (row, index, next) {
        if ($(row).find('.cart_item_desc_ext').length) {
          return next(
    ...
    function add1ClickButton () {
      if ($('.already_in_library').length) {
        return;
      }

      $('.game_area_purchase_game_wrapper').each(function (index, elem) {
        var gameWrapper = $(elem);
        var form = gameWrapper.find('form');
        ...
        if (
          ...
          || gameWrapper.find('.game_area_purchase_not_refundable').length
        ) {
          gameWrapper
            .find('.game_area_purchase_not_refundable')
            .append(
        ...
        var addToCartButtonParent = gameWrapper
          .find('.game_purchase_action_bg:last .btn_addtocart')
          .parent();
        $(addToCartButtonParent)
          .append($('<div class="oneclick-delim">or</div>'));
        $(addToCartButtonParent).append(button);
      });
    }
    ...
    if (
      pathname === '/checkout/'
      && search.match(/purchasetype=self/) !== null
    ) {
      do1ClickPhaseThree();
    }
